...
std::string parseShaderFile(const std::string& filepath) {
    ...
    if (!file) {
        std::cerr << "Failed to open shader file (C-style): " << filepath << std::endl;
        exit(-1);
    }
...
public:
    ...
    {
        std::string vertexCode = parseShaderFile(vertexPath);
        std::string fragmentCode = parseShaderFile(fragmentPath);
